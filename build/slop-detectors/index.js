...
import { detectContextualSlop, detectShortHookParagraph, detectTripleConstruction, detectVerbIntensifierForms, } from './nlp-patterns.js';
import { detectAlmostHedge, detectAnaphoraAbuse, detectBoldFirstBullets, detectBroaderImplications, detectColonElaboration, detectConceptLabel, detectConnectorAddiction, detectDespiteChallenges, detectDramaticFragment, detectElevatedRegister, detectEmDashPivot, detectEraOpener, detectFalseConclusion, detectFalseRange, detectFillerAdverbs, detectGerundLitany, detectHedgeStack, detectHeresTheKicker, detectHighlightSlop, detectImagineWorld, detectImportantToNote, detectListicleInstinct, detectListicleTrenchCoat, detectMetaphorCrutch, detectNegationCountdown, detectNegationPivot, detectOverusedIntensifiers, detectParentheticalQualifier, detectPedagogicalAside, detectQuestionThenAnswer, detectServesAs, detectStaccatoBurst, detectSuperficialAnalysis, detectUnicodeArrows, detectUnnecessaryContrast, detectVagueAttribution, } from './word-patterns.js';
const articleProcessor = retext().use(retextIndefiniteArticle);
export function runClientDetectors(text) {
    const all = [
...
function fixArticleContext(violations, text) {
    return violations.map((v) => {
        if (v.suggestedChange === null)
            return v;
        ...
        if (!file.messages.length)
            return v;
        const correctArticle = file.messages[0].expected?.[0];
        if (!correctArticle)
            return v;
...
function deduplicateViolations(violations) {
    ...
    return violations.filter((v) => {
        ...
        if (seen.has(key))
            return false;
        seen.add(key);
        const containedByLarger = violations.some((other) => other !== v &&
