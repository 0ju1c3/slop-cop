 ...
 */
function extractSentenceAt(text, pos) {
    let start = pos;
    while (start > 0 && !/[.!?\n]/.test(text[start - 1]))
        start--;
    ...
    if (end < text.length)
        end++; // include the terminating punctuation/newline
    return { text: text.slice(start, end), offset: start };
}
/** Conjugate baseVerb to match the tense tags of a detected verb term */
function conjugate(baseVerb, tags) {
    const conj = nlp(baseVerb).verbs().conjugate()[0];
    if (!conj)
        return baseVerb;
    if (tags.includes('Gerund'))
        return conj.Gerund ?? baseVerb;
    if (tags.includes('PastTense'))
        return conj.PastTense ?? baseVerb;
    ...
    if (tags.includes('PresentTense'))
        return conj.PresentTense ?? baseVerb;
    return conj.Infinitive ?? baseVerb;
}
/**
 ...
 */
function firstTermViolations(doc, pattern, ruleId) {
    const violations = [];
    doc.match(pattern).forEach((m) => {
        const matches = m.json({ offset: true });
        if (!matches.length)
            return;
 ...
 */
function inAWayViolations(doc, _chunkText, ruleId) {
    const violations = [];
    doc
        .match('in (a|an) #Adjective (way|manner|sense|fashion|regard)')
        .forEach((m) => {
        const matches = m.json({ offset: true, tags: true });
        if (!matches.length)
            return;
        ...
        const adjTerm = (phrase.terms ?? []).find((t) => t.tags.includes('Adjective'));
        if (!adjTerm)
            return;
...
const OVERUSED_VERB_RE = new RegExp(`^(${OVERUSED_VERB_STEMS.map(toStemPrefix).join('|')})`, 'i');
...
function addS(verb) {
    if (verb === 'have')
        return 'has'; // irregular: boast → have → has
    if (verb.endsWith('y') && !/[aeiou]y$/i.test(verb))
        return `${verb.slice(0, -1)}ies`;
    if (/([sxz]|[sc]h)$/i.test(verb))
        return `${verb}es`;
    return `${verb}s`;
}
function addIng(verb) {
    if (verb === 'get')
        return 'getting'; // irregular: garner → get → getting
    if (verb.endsWith('e'))
 ...
 */
export function detectVerbIntensifierForms(text) {
    const violations = [];
    for (const stem of OVERUSED_VERB_STEMS) {
        const replacement = VERB_REPLACEMENTS[stem];
        if (replacement === undefined)
            continue; // no clean swap (delve, embark, resonate)
...
function runNlpDetectors(doc, chunkText) {
    const v = [];
...
export function detectContextualSlop(text) {
 ...
 */
export function detectShortHookParagraph(text) {
    const violations = [];
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (restAvg < firstWords * 1.5 || restAvg < 12)
            continue;
...
// ── Triple construction ───────────────────────────────────────────────────────
export function detectTripleConstruction(text) {
    if (!text.includes(','))
        return [];
    ...
    function endsWithProperNoun(t) {
        return nlp(`x ${t.trim()}`).terms().last().has('#ProperNoun');
    }
    function startsWithProperNoun(t) {
    ...
    for (const { text: chunk, offset } of windows.values()) {
        ...
        while ((m = oxfordRe.exec(chunk)) !== null) {
            if (CLAUSE_OPENER.test(m[2].trimStart()) ||
                CLAUSE_OPENER.test(m[3].trimStart()))
                continue;
            if (endsWithProperNoun(m[1]) && !startsWithProperNoun(m[2]))
                continue; // named-entity appositive
            violations.push({
