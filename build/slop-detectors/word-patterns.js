...
function findAll(text, pattern, ruleId) {
    const violations = [];
    let m;
...
export function detectHighlightSlop(text) {
    const violations = [];
    const re = /\b(highlights?|highlighted|highlighting)\s+the\s+(\w+)/gi;
    let m;
    while ((m = re.exec(text)) !== null) {
        if (!HIGHLIGHT_ABSTRACT_NOUNS.test(m[2]))
            continue;
        const verbText = m[1];
        let suggestion;
...
export function detectOverusedIntensifiers(text) {
    const violations = [];
...
export function detectElevatedRegister(text) {
    const violations = [];
...
export function detectFillerAdverbs(text) {
    const violations = [];
...
export function detectAlmostHedge(text) {
...
export function detectImportantToNote(text) {
...
export function detectBroaderImplications(text) {
...
export function detectFalseConclusion(text) {
    const violations = [];
...
export function detectConnectorAddiction(text) {
...
export function detectUnnecessaryContrast(text) {
    const violations = [];
...
export function detectEmDashPivot(text) {
    const violations = [];
...
export function detectNegationPivot(text) {
    const violations = [];
    ...
    const semicolonPivotRe = new RegExp(`\\b(${NEGATIONS})\\b[^;.!?\\n]{3,80};`, 'gi');
    ...
    const twoSentenceRe = new RegExp(`(([A-Z][\\w\u2019']*)\\s+${NEG}\\b[^.!?\\n]{5,120}[.!?])[ \\t]+(\\2\\b[^.!?\\n]{5,120}[.!?])`, 'g');
...
export function detectColonElaboration(text) {
...
export function detectParentheticalQualifier(text) {
...
export function detectQuestionThenAnswer(text) {
    const violations = [];
...
export function detectHedgeStack(text) {
...
export function detectStaccatoBurst(text) {
    const violations = [];
    const paragraphs = splitParagraphs(text);
    for (const para of paragraphs) {
        ...
        while (i < sentences.length) {
            if (wordCount(sentences[i]) <= 8) {
                let j = i + 1;
                while (j < sentences.length && wordCount(sentences[j]) <= 8)
                    j++;
                if (j - i >= 3) {
...
export function detectListicleInstinct(text) {
    const violations = [];
...
export function splitParagraphs(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        if (chunk.trim())
            results.push({ text: chunk, start: last });
...
export function detectAnaphoraAbuse(text) {
    const violations = [];
    const sentences = splitSentences(text);
    let offset = 0;
    const offsets = [];
    ...
    function twoWordOpener(s) {
        ...
        if (skip.has(first) || first.length < 2)
            return '';
        return `${first} ${words[1].toLowerCase().replace(/[^a-z]/g, '')}`;
    }
    function singleWordOpener(s) {
        const words = normalize(s);
        if (words.length < 2)
            return '';
        const first = words[0].toLowerCase().replace(/[^a-z]/g, '');
        if (first.length < 2 || ANAPHORA_SINGLE_WORD_SKIP.has(first))
            return '';
        return first;
    }
    function flagRun(i, j, opener) {
        const start = offsets[i];
        const end = offsets[j - 1] + sentences[j - 1].length;
        violations.push({
    ...
    while (i < sentences.length) {
        ...
        if (two) {
            let j = i + 1;
            while (j < sentences.length && twoWordOpener(sentences[j]) === two)
                j++;
            if (j - i >= 3) {
                flagRun(i, j, two);
                i = j;
        ...
        if (one) {
            let j = i + 1;
            while (j < sentences.length && singleWordOpener(sentences[j]) === one)
                j++;
            if (j - i >= 3) {
                flagRun(i, j, one);
                i = j;
...
export function detectGerundLitany(text) {
    const violations = [];
    const sentences = splitSentences(text);
    let offset = 0;
    const offsets = [];
    ...
    while (i < sentences.length) {
        if (isGerund(sentences[i])) {
            let j = i + 1;
            while (j < sentences.length && isGerund(sentences[j]))
                j++;
            if (j - i >= 2) {
                const start = offsets[i];
                const end = offsets[j - 1] + sentences[j - 1].length;
...
export function detectHeresTheKicker(text) {
    const violations = [];
...
export function detectPedagogicalAside(text) {
    const violations = [];
    const lower = text.toLowerCase();
...
export function detectVagueAttribution(text) {
    const violations = [];
    const lower = text.toLowerCase();
...
export function detectBoldFirstBullets(text) {
    const re = /^[ \t]*[-*•][ \t]+\*\*[^*\n]+\*\*/gm;
    return findAll(text, re, 'bold-first-bullets');
}
export function detectUnicodeArrows(text) {
    return findAll(text, /→/g, 'unicode-arrows');
}
export function detectDespiteChallenges(text) {
...
export function detectDramaticFragment(text) {
    ...
    for (const para of splitParagraphs(text)) {
        ...
        if (wordCount >= 1 && wordCount <= 4 && !trimmed.endsWith(':')) {
            ...
            if (!hasTerminalPunct && (isFirstPara || allWordsCapped))
                continue;
            violations.push({
...
export function detectSuperficialAnalysis(text) {
...
export function detectFalseRange(text) {
    const violations = [];
