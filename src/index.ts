...
function buildLLMDetectionPrompt(): string {
  ...
  const formatRules = (rules: typeof RULES) =>
    rules
      .map(
        (r, i) => `${i + 1}. "${r.id}": ${r.llmDetectionHint ?? r.description}`,
      )
      .join('\n\n')
...
server.registerTool(
  ...
  async ({ text }) => {
    ...
    const directFixes = violations
      .filter(
        (v): v is typeof v & { suggestedChange: string } =>
          typeof v.suggestedChange === 'string',
      )
    ...
    const remaining = runClientDetectors(fixed).filter(
      (v) => v.suggestedChange == null,
    )

    if (remaining.length === 0) {
      return { content: [{ type: 'text' as const, text: fixed }] }
    }
    ...
    for (const para of paragraphs) {
      const paraViolations = remaining.filter(
      ...
      for (const [ruleId, matches] of byRule) {
        const directive =
          RULES_BY_ID[ruleId]?.llmDirective ?? RULES_BY_ID[ruleId]?.tip
      ...
      try {
        const rewritten = await sample(
          buildRewriteSystemPrompt(ruleHints),
          para.text,
          2048,
        )
    ...
    return {
      content: [{ type: 'text' as const, text: rewrittenParts.join('\n\n') }],
    }
  },
)
